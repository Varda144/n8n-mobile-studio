...
class N8nProcess(private val ctx:Context){
    ...
    fun start(extraEnv:Map<String,String>=emptyMap()): Result<Unit> {
        return try{
            ...
            val bin = File(n8nDir,"node_modules/.bin/n8n")
            if(!bin.exists()) return Result.failure(IllegalStateException("n8n not installed"))
            bin.setExecutable(true)
            val dataDir=paths.n8nData.absolutePath
            File(dataDir).mkdirs()
            val env = mutableMapOf(
                "N8N_PORT" to com.n8n.mobile.studio.core.AppConfig.N8N_PORT.toString(),
                "N8N_DATA_FOLDER" to dataDir,
                "N8N_PROTOCOL" to "http",
                "N8N_HOST" to "127.0.0.1",
                "NODE_ENV" to "production",
                "N8N_SECURE_COOKIE" to "false"
            ) + extraEnv
            val pb = ProcessBuilder(bin.absolutePath, "start")
            pb.directory(n8nDir)
            val e=pb.environment()
            e.putAll(node.env()); e.putAll(env)
            pb.redirectErrorStream(true)
            proc=pb.start()
            Logger.d("n8n started pid ${proc?.pid()}")
            Result.success(Unit)
        }catch(e:Exception){ Logger.e("N8nProcess start fail",e); Result.failure(e)}
    }
